...
const SingleChoice = () => {
    ...
    const removeItem = index => {
        ...
        if (currentCorrect && !nextItems.some(item => item.prefix === currentCorrect)) {
            form.setFieldsValue({ correct: undefined })
        }
    }
        ...
        <div className="app-container exam-editor-page">
            ...
            <Form className="exam-editor-form" form={form} layout="vertical" onFinish={onFinish}>
                ...
                <Form.Item name="correct" label="正确答案" rules={[{ required: true, message: '请选择正确答案' }]}>
                    <Radio.Group>
                        {items.map(item => <Radio key={item.prefix} value={item.prefix}>{item.prefix}</Radio>)}
                    </Radio.Group>
                </Form.Item>
