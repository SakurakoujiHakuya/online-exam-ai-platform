...
public class TaskController extends BaseApiController {
    ...
    @Autowired
    public TaskController(TaskExamService taskExamService, SubjectService subjectService) {
    ...
    public RestResponse<PageInfo<TaskPageResponseVM>> pageList(@RequestBody TaskPageRequestVM model) {
        ...
        PageInfo<TaskPageResponseVM> page = PageInfoHelper.copyMap(pageInfo, m -> {
            ...
            vm.setStartTime(DateTimeUtil.dateFormat(m.getStartTime()));
            vm.setEndTime(DateTimeUtil.dateFormat(m.getEndTime()));
            vm.setUserGroupName(subjectService.userGroupNameById(m.getUserGroupId()));
            return vm;
        });
