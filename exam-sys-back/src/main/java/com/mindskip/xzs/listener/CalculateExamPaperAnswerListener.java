...
public class CalculateExamPaperAnswerListener implements ApplicationListener<CalculateExamPaperAnswerCompleteEvent> {
    ...
    private final static Logger logger = LoggerFactory.getLogger(CalculateExamPaperAnswerListener.class);
     ...
     * @param examPaperQuestionCustomerAnswerService the exam paper question
     *                                               customer answer service
     * @param textContentService                     the text content service
     * @param examCustomerAnswerService              the exam customer answer
     *                                               service
     */
    @Autowired
    public CalculateExamPaperAnswerListener(ExamPaperAnswerService examPaperAnswerService,
            ...
            TextContentService textContentService, TaskExamCustomerAnswerService examCustomerAnswerService) {
    ...
    public void onApplicationEvent(CalculateExamPaperAnswerCompleteEvent calculateExamPaperAnswerCompleteEvent) {
        try {
            ...
            ExamPaperAnswerInfo examPaperAnswerInfo = (ExamPaperAnswerInfo) calculateExamPaperAnswerCompleteEvent
                    .getSource();
            ExamPaper examPaper = examPaperAnswerInfo.getExamPaper();
            ExamPaperAnswer examPaperAnswer = examPaperAnswerInfo.getExamPaperAnswer();
            List<ExamPaperQuestionCustomerAnswer> examPaperQuestionCustomerAnswers = examPaperAnswerInfo
            ...
            examPaperQuestionCustomerAnswers.stream().filter(a -> QuestionTypeEnum.needSaveTextContent(a.getQuestionType()))
                    .forEach(d -> {
            ...
            examPaperQuestionCustomerAnswers.forEach(d -> {
                ...
                if (d.getDoRight() == null && !QuestionTypeEnum.ShortAnswer.equals(QuestionTypeEnum.fromCode(d.getQuestionType()))) {
                    d.setDoRight(false);
                }
                if (d.getDoTime() == null) {
                    d.setDoTime(0);
            ...
            if (examPaper.getTaskExamId() != null
                    || ExamPaperTypeEnum.fromCode(examPaper.getPaperType()) == ExamPaperTypeEnum.Task) {
                examCustomerAnswerService.insertOrUpdate(examPaper, examPaperAnswer, now);
            }
        } catch (Exception e) {
            logger.error("Error in CalculateExamPaperAnswerListener: " + e.getMessage(), e);
